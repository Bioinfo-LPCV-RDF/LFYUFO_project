...
# results=/home/312.6-Flo_Re/312.6.1-Commun/LFY/LFY-UFO/results
...
# # 	sample_names1=("LFYamp1" "LFYamp2" "LFYamp3")
# # 	sample_names2=("LFYUFO_cfa" "LFYUFO_cfb" "LFYUFO_cfc")
...
# 	
# 	## LFYamp
# # 	get_RIP_RIL_bdgs -n ${name1} -p $peaks -bam $mapping_dir -sn sample_names1[@] -od $output -pd $dir_peakcalling -m "inPeaks" -s 123
# # 	rm $output/inPeaks/${name1}/${sample_names1[0]}_RIP_cov.bdg
# # 	rm $output/inPeaks/${name1}/${sample_names1[1]}_RIP_cov.bdg
# # 	rm $output/inPeaks/${name1}/${sample_names1[2]}_RIP_cov.bdg
# 	
# 	## LFYUFO_cf
# # 	get_RIP_RIL_bdgs -n ${name2} -p $peaks -bam $mapping_dir -sn sample_names2[@] -od $output -pd $dir_peakcalling -m "inPeaks" -s 123
# # 	rm $output/inPeaks/${name2}/${sample_names2[0]}_RIP_cov.bdg
# # 	rm $output/inPeaks/${name2}/${sample_names2[1]}_RIP_cov.bdg
# # 	rm $output/inPeaks/${name2}/${sample_names2[2]}_RIP_cov.bdg
# 	
# 	## LFYm
...
if [[ ! -f $dir_comparisons/LFYamp_LFYUFO_LFYm_LFYmUFO/violinplot_top20perc_CFC_pub.png ]]; then
	conda run -n LFYUFO_figs Rscript LFY_mutant_plots_clean.r "$dir_comparisons/LFYm_LFYmUFO_cf" "$dir_comparisons/LFYamp_LFYm" "$dir_comparisons/LFYamp_LFYUFO_LFYm_LFYmUFO"
fi
exit 0
...
# # # # # grep "AT5G20240" $gene_positions | awk -v OFS="\t" '{print $1,$2-1000,$2}' > $out_PI/PI_prom.bed  #### old
