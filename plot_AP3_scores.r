...
# setwd("//oslo/share_nfs/312-Projets_PCV/312.6-Flo_Re/312.6.1-Commun/LFY/LFY-UFO/results/comparisons/LFYamp_LFYUFO_cf/AP3")
# # setwd(wd)
...
vlines <- function(x, y, y.orig,y.max,col){
  plot(range(x), range(c(y, y.orig)), type='n',
       ylim = c(y.orig,y.max),xlim = c(0,930),#c(-975,0),
       yaxt="n",xaxt="n",
       axes = FALSE,
  ...
  # custom x axis
  xtick<-c(0,930) #seq(0,-975, by=-500)
